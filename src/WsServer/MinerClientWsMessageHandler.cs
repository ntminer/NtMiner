    ...
    public static class MinerClientWsMessageHandler {
        private static readonly Dictionary<string, Action<MinerClientBehavior, string, Guid, WsMessage>>
            _handlers = new Dictionary<string, Action<MinerClientBehavior, string, Guid, WsMessage>>(StringComparer.OrdinalIgnoreCase) {
                [WsMessage.ConsoleOutLines] = (wsBehavior, loginName, clientId, message) => {
                ...
                [WsMessage.OperationReceived] = (wsBehavior, loginName, clientId, message) => {
                    WsRoot.OperationMqSender.SendOperationReceived(loginName, clientId);
                },
                [WsMessage.Speed] = (wsBehavior, loginName, clientId, message) => {
                ...
                [WsMessage.SelfWorkLocalJson] = (wsBehavior, loginName, clientId, message) => {
                    if (message.TryGetData(out string json)) {
                        WsRoot.OperationMqSender.SendSelfWorkLocalJson(loginName, clientId, json);
                    }
                },
                [WsMessage.GpuProfilesJson] = (wsBehavior, loginName, clientId, message) => {
                    if (message.TryGetData(out string json)) {
