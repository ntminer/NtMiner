...
namespace NTMiner.Data.Impl {
    public class CoinSnapshotSet : ICoinSnapshotSet {
        ...
        private void Snapshot(DateTime leftTime, int seconds) {
            ...
            Global.DebugLine($"快照时间区间{{{leftTime} - {rightTime}]", ConsoleColor.Yellow);
            if (rightTime <= DateTime.Now) {
                ...
                Global.DebugLine("快照已拍摄到当前时间", ConsoleColor.Yellow);
            }
        }
        ...
        private void Snapshot(List<ClientCoinSnapshotData> clientCoinSpeedList, DateTime timestamp) {
            ...
            if (data.Count > 0) {
                ...
                Global.DebugLine("拍摄快照" + data.Count + "张，快照时间戳：" + timestamp.ToString("yyyy-MM-dd HH:mm:ss fff"), ConsoleColor.Gray);
            }
        }
