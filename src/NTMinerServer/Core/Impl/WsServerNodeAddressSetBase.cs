...
namespace NTMiner.Core.Impl {
    public class WsServerNodeAddressSetBase : IWsServerNodeAddressSet {
        ...
        public WsServerNodeAddressSetBase(IReadOnlyWsServerNodeRedis wsServerNodeRedis) {
            ...
            VirtualRoot.BuildEventPath<Per20SecondEvent>("周期从redis中刷新WsServer节点集", LogEnum.DevConsole, this.GetType(), PathPriority.Normal, path: message => {
                Init();
            });
            // 收到Mq消息之前一定已经初始化完成，因为Mq消费者在WsServerNodeAddressSetInitedEvent事件之后才会创建
            VirtualRoot.BuildEventPath<WsServerNodeRemovedMqEvent>("收到移除了服务器节点Mq消息后刷新节点列表", LogEnum.UserConsole, this.GetType(), PathPriority.Normal, path: message => {
                if (message.AppId == ServerRoot.HostConfig.ThisServerAddress) {
                    return;
                }
                Init();
            });
            VirtualRoot.BuildEventPath<WsServerNodeAddedMqEvent>("收到添加了服务器节点Mq消息后刷新节点列表", LogEnum.UserConsole, this.GetType(), PathPriority.Normal, path: message => {
                if (message.AppId == ServerRoot.HostConfig.ThisServerAddress) {
                    return;
                }
                Init();
