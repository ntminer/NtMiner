    ...
    internal class NVIDIAGpuSet : IGpuSet {
        ...
        public NVIDIAGpuSet(INTMinerRoot root) {
            ...
            if (gpus.Count > 0) {
                ...
                try {
                    var item = root.SysDicItemSet.GetSysDicItems(VirtualRoot.CudaVersionSysDicCode)
                        .Select(a => new { Version = double.Parse(a.Value), a })
                        .OrderByDescending(a => a.Version)
                        .FirstOrDefault(a => _driverVersion.Major >= a.Version);
                    if (item != null) {
                        this.Properties.Add(new GpuSetProperty(VirtualRoot.CudaVersionSysDicCode, "Cuda版本", item.a.Code));
                    }
                }
                catch (Exception e) {
