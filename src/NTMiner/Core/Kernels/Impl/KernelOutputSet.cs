...
namespace NTMiner.Core.Kernels.Impl {
    public class KernelOutputSet : IKernelOutputSet {
        ...
        public KernelOutputSet(INTMinerRoot root) {
            _root = root;
            #region 接线
            _root.ServerContext.BuildCmdPath<AddKernelOutputCommand>("添加内核输出组", LogEnum.DevConsole,
                action: (message) => {
                    InitOnece();
            ...
            _root.ServerContext.BuildCmdPath<UpdateKernelOutputCommand>("更新内核输出组", LogEnum.DevConsole,
                action: (message) => {
                    InitOnece();
                    if (message == null || message.Input == null || message.Input.GetId() == Guid.Empty) {
                        throw new ArgumentNullException();
            ...
            _root.ServerContext.BuildCmdPath<RemoveKernelOutputCommand>("移除内核输出组", LogEnum.DevConsole,
                action: (message) => {
                    InitOnece();
                    ...
                    IKernel[] outputUsers = root.ServerContext.KernelSet.Where(a => a.KernelOutputId == message.EntityId).ToArray();
                    ...
                    List<Guid> kernelOutputTranslaterIds = root.ServerContext.KernelOutputTranslaterSet.Where(a => a.KernelOutputId == entity.Id).Select(a => a.GetId()).ToList();
