        ...
        public static void BlockWAU() {
            try {
                Task.Factory.StartNew(() => {
                    ...
                    VirtualRoot.ThisWorkerMessage(nameof(WindowsUtil), WorkerMessageType.Info, "禁用windows系统更新成功", toOut: true);
                });
            }
            catch (Exception e) {
                VirtualRoot.ThisWorkerMessage(nameof(WindowsUtil), WorkerMessageType.Error, "禁用windows系统更新失败", toOut: true);
        ...
        public static void Win10Optimize() {
            try {
                Task.Factory.StartNew(() => {
                    ...
                    VirtualRoot.ThisWorkerMessage(nameof(WindowsUtil), WorkerMessageType.Info, "优化Windows成功", toOut: true);
                });
            }
            catch (Exception e) {
                Logger.ErrorDebugLine(e);
                VirtualRoot.ThisWorkerMessage(nameof(WindowsUtil), WorkerMessageType.Error, "优化Windows失败", toOut: true);
            }
        }
    }
}
