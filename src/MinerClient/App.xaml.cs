                ...
                if (createdNew) {
                    ...
                    NTMinerRoot.Instance.Init(() => {
                        ...
                        if (NTMinerRoot.Instance.GpuSet.Count == 0) {
                            VirtualRoot.Ui.ShowErrorMessage("没有矿卡或矿卡未驱动。");
                        }
                        if (NTMinerRoot.Instance.CoinSet.Count == 0) {
                            VirtualRoot.Ui.ShowErrorMessage("访问阿里云失败，更换本机dns可以解决此问题");
                        }
                        UIThread.Execute(() => {
                            if (NTMinerRoot.GetIsNoUi() && NTMinerRegistry.GetIsAutoStart()) {
                                VirtualRoot.Ui.ShowSuccessMessage("已切换为无界面模式运行，可在选项页调整设置", "开源矿工");
                            }
                            else {
            ...
            VirtualRoot.Window<CloseMainWindowCommand>("处理关闭主界面命令", LogEnum.DevConsole,
                action: message => {
                    UIThread.Execute(() => {
                        ...
                        VirtualRoot.Ui.ShowSuccessMessage(message.Message, "开源矿工");
                    });
                });
            #region 周期确保守护进程在运行
