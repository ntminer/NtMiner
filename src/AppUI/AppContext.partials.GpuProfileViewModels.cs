            ...
            private GpuProfileViewModels() {
                On<GpuProfileSetRefreshedEvent>("Gpu超频集合刷新后刷新附着在当前币种上的超频数据", LogEnum.DevConsole,
                    action: message => {
                        ...
                        var coinVm = Current.MinerProfileVm.CoinVm;
                        if (coinVm != null) {
                        ...
                        lock (_locker) {
                            ...
                            if (_listByCoinId.TryGetValue(message.Source.CoinId, out list)) {
                                ...
                                if (vm != null) {
                                    ...
                                    if (Current.GpuVms.TryGetGpuVm(message.Source.Index, out GpuViewModel gpuVm)) {
            ...
            public GpuProfileViewModel GpuAllVm(Guid coinId) {
                ...
                if (!_gpuAllVmDicByCoinId.TryGetValue(coinId, out result)) {
                    lock (_locker) {
                        if (!_gpuAllVmDicByCoinId.TryGetValue(coinId, out result)) {
                            GpuViewModel gpuVm;
                            Current.GpuVms.TryGetGpuVm(NTMinerRoot.GpuAllId, out gpuVm);
            ...
            public List<GpuProfileViewModel> List(Guid coinId) {
                ...
                if (!_listByCoinId.TryGetValue(coinId, out list)) {
                    lock (_locker) {
                        if (!_listByCoinId.TryGetValue(coinId, out list)) {
                            list = new List<GpuProfileViewModel>();
                            foreach (var gpu in Current.GpuVms) {
