    ...
    public class DaemonWsClient : AbstractWsClient {
        ...
        protected override void UpdateWsStateAsync(string description, bool toOut) {
            if (VirtualRoot.DaemonOperation.IsNTMinerOpened()) {
                ...
                state.ToOut = toOut;
                JsonRpcRoot.FirePostAsync(NTKeyword.Localhost, NTKeyword.MinerClientPort, JsonRpcRoot.GetControllerName<IMinerClientController>(), nameof(IMinerClientController.ReportWsDaemonState), null, state, timeountMilliseconds: 3000);
            }
        }
    }
}
