...
namespace NTMiner {
    ...
    public class NTMinerDaemonController : ApiController, INTMinerDaemonController {
        ...
        public ResponseBase EnableWindowsRemoteDesktop() {
            try {
                VirtualRoot.WorkerEvent(WorkerEventChannel.MinerStudio, $"{VirtualRoot.WorkerEventFromMinerStudio}启用Windows远程桌面");
        ...
        public void CloseDaemon() {
            VirtualRoot.WorkerEvent(WorkerEventChannel.MinerClient, $"{VirtualRoot.WorkerEventFromMinerStudio}退出守护进程");
        ...
        public string GetGpuProfilesJson() {
            try {
                VirtualRoot.WorkerEvent(WorkerEventChannel.MinerStudio, $"{VirtualRoot.WorkerEventFromMinerStudio}获取显卡参数");
        ...
        public void SaveGpuProfilesJson() {
            try {
                VirtualRoot.WorkerEvent(WorkerEventChannel.MinerStudio, $"{VirtualRoot.WorkerEventFromMinerStudio}保存显卡参数");
        ...
        public void SetAutoBootStart([FromUri]bool autoBoot, [FromUri]bool autoStart) {
            VirtualRoot.WorkerEvent(WorkerEventChannel.MinerStudio, $"{VirtualRoot.WorkerEventFromMinerStudio}开机启动{(autoBoot ? "√": "×")}，自动挖矿{(autoStart ? "√" : "×")}");
        ...
        public ResponseBase RestartWindows([FromBody]SignRequest request) {
            ...
            try {
                VirtualRoot.WorkerEvent(WorkerEventChannel.MinerStudio, $"{VirtualRoot.WorkerEventFromMinerStudio}重启矿机");
        ...
        public ResponseBase ShutdownWindows([FromBody]SignRequest request) {
            ...
            try {
                VirtualRoot.WorkerEvent(WorkerEventChannel.MinerStudio, $"{VirtualRoot.WorkerEventFromMinerStudio}关机");
        ...
        public ResponseBase RestartNTMiner([FromBody]WorkRequest request) {
            ...
            VirtualRoot.WorkerEvent(WorkerEventChannel.MinerStudio, $"{VirtualRoot.WorkerEventFromMinerStudio}重启挖矿端");
            if (request.WorkId != Guid.Empty) {
                File.WriteAllText(SpecialPath.NTMinerLocalJsonFileFullName, request.LocalJson);
                File.WriteAllText(SpecialPath.NTMinerServerJsonFileFullName, request.ServerJson);
            }
        ...
        public ResponseBase UpgradeNTMiner([FromBody]UpgradeNTMinerRequest request) {
            ...
            VirtualRoot.WorkerEvent(WorkerEventChannel.MinerStudio, $"{VirtualRoot.WorkerEventFromMinerStudio}升级挖矿端至{request.NTMinerFileName}");
            Task.Factory.StartNew(() => {
                try {
                    string location = NTMinerRegistry.GetLocation();
                    if (!string.IsNullOrEmpty(location) && File.Exists(location)) {
