    ...
    public static partial class Server {
        public class ControlCenterServiceFace {
            ...
            public void GetServicesVersionAsync(Action<string, Exception> callback) {
                Process[] processes = Process.GetProcessesByName("NTControlCenterServices");
                ...
                Task.Factory.StartNew(() => {
                    try {
                        using (HttpClient client = new HttpClient()) {
                            Task<HttpResponseMessage> message = client.PostAsync($"http://localhost:{NTKeyword.OfficialServerPort}/api/{SControllerName}/{nameof(IControlCenterController.GetServicesVersion)}", null);
            ...
            public void CloseServices() {
                try {
                    ...
                    using (HttpClient client = new HttpClient()) {
                        Task<HttpResponseMessage> message = client.PostAsync($"http://localhost:{NTKeyword.OfficialServerPort}/api/{SControllerName}/{nameof(IControlCenterController.CloseServices)}", null);
