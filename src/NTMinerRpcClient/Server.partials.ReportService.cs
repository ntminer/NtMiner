                ...
                Task.Factory.StartNew(() => {
                    ...
                    try {
                        using (HttpClient client = new HttpClient()) {
                            ...
                            Task<HttpResponseMessage> message = client.PostAsJsonAsync($"http://{host}:{NTKeyword.OfficialServerPort}/api/{SControllerName}/{nameof(IReportController.ReportSpeed)}", data);
                ...
                Task.Factory.StartNew(() => {
                    ...
                    try {
                        using (HttpClient client = new HttpClient()) {
                            ...
                            Task<HttpResponseMessage> message = client.PostAsJsonAsync($"http://{host}:{NTKeyword.OfficialServerPort}/api/{SControllerName}/{nameof(IReportController.ReportState)}", request);
