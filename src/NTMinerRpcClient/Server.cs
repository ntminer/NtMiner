            ...
            Task.Factory.StartNew(() => {
                try {
                    ...
                    using (HttpClient client = new HttpClient()) {
                        Task<HttpResponseMessage> message = client.PostAsJsonAsync($"http://{serverHost}:{VirtualRoot.ControlCenterPort}/api/{controller}/{action}{queryString}", param);
                ...
                try {
                    ...
                    using (HttpClient client = new HttpClient()) {
                        ...
                        Task<HttpResponseMessage> message =
                            client.GetAsync($"http://{serverHost}:{VirtualRoot.ControlCenterPort}/api/{controller}/{action}{queryString}");
                        T response = message.Result.Content.ReadAsAsync<T>().Result;
                        callback?.Invoke(response, null);
                    }
                }
